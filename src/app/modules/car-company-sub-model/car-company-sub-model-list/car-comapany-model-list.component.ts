...
export class CarCompanySubModelListComponent implements OnInit {
  ...
  onDelete(carCompany: { id: any; }) {
    ...
    modalRef.content?.onClose.subscribe(async (isConfirmed: any) => {
      if (isConfirmed) {
        await this.httpService.delete(`/car/car-sub-model/${carCompany.id}`);
        _.remove(this.carCompanys, (n: { id: any; }) => {
          return n.id === carCompany.id;
        });
        this.toastr.success(this.translation.getLocalizedString('DELETE_CARCOMPANYSUBMODEL_SUCCESFULLY'));
      }
    });
  }

  public async fetch() {
    const res = await this.httpService.get('/car/all-car-sub-model', {}) as any;
